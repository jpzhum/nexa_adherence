...
def ensure_schema(conn: Optional[sqlite3.Connection] = None) -> None:
    """
    Inicializa o schema e aplica migrações incrementais com controle de versão.
    """
    ...
    try:
        ...
        if current_version is None:
            logger.info("Criando schema versão %s", SCHEMA_VERSION)
            _apply_migration_1(conn)
            _set_version(conn, SCHEMA_VERSION)
        elif current_version < SCHEMA_VERSION:
            logger.info("Migrando schema de %s para %s", current_version, SCHEMA_VERSION)
            _run_migrations(conn, current_version)
        else:
            logger.debug("Schema já na versão %s", current_version)
    finally:
        if owns_conn:
            conn.close()
...
def _run_migrations(conn: sqlite3.Connection, current_version: int) -> None:
    """
    Placeholder para futuras versões; migra incrementalmente até SCHEMA_VERSION.
    """
    # Como SCHEMA_VERSION = 1, não há migrações intermediárias ainda.
    _set_version(conn, SCHEMA_VERSION)


def _apply_migration_1(conn: sqlite3.Connection) -> None:
    """
    Cria tabelas iniciais com deduplicação por arquivo (imports.file_hash) e chaves naturais básicas.
    """
    conn.execute(
        """
